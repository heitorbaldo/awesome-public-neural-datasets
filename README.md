# Awesome Public Neural Datasets [![Awesome](https://awesome.re/badge.svg)](https://github.com/sindresorhus/awesome)

A curated list of awesome publicly available neural datasets.

A curated list of publicly available datasets and data repositories for human and animal neuroscience, covering neuroimaging (MRI, fMRI, DWI/DTI), electrophysiology (EEG, MEG, iEEG/ECoG, single-unit), metabolic imaging (PET), and brain stimulation (TMS, tDCS).

## Contents

- [General & Multimodal Repositories](#general--multimodal-repositories)
- [Standards & Data Formats](#standards--data-formats)
- [Structural MRI](#structural-mri)
- [Functional MRI (fMRI)](#functional-mri-fmri)
- [Diffusion MRI (DWI / DTI / Tractography / TDI)](#diffusion-mri-dwi--dti--tractography--tdi)
- [PET & Metabolic Imaging](#pet--metabolic-imaging)
- [EEG](#eeg)
- [MEG](#meg)
- [Intracranial EEG (iEEG / ECoG / SEEG)](#intracranial-eeg-ieeg--ecog--seeg)
- [Single-Unit & Extracellular Electrophysiology](#single-unit--extracellular-electrophysiology)
- [Brain Stimulation (TMS / tDCS / TMS-EEG)](#brain-stimulation-tms--tdcs--tms-eeg)
- [fNIRS](#fnirs)
- [Calcium & Optical Imaging](#calcium--optical-imaging)
- [Atlases & Connectomes](#atlases--connectomes)
- [Clinical & Disease-Specific Collections](#clinical--disease-specific-collections)
- [License](#license)

<hr>

## General & Multimodal Repositories

## Standards & Data Formats

- [BIDS (Brain Imaging Data Structure)](https://bids.neuroimaging.io) - Standard format for MRI, fMRI, MEG, EEG, iEEG, and PET datasets.
- [NWB (Neurodata Without Borders)](https://www.nwb.org) - Standard format for cellular-level neurophysiology (spikes, LFP, imaging).
- [BIDS Examples](https://github.com/bids-standard/bids-examples) - Reference datasets illustrating the BIDS specification.

## Structural MRI

## Functional MRI (fMRI)

## Diffusion MRI (DWI / DTI / Tractography / TDI)

## PET & Metabolic Imaging

## EEG

## MEG

## Intracranial EEG (iEEG / ECoG / SEEG)

- [IEEG.org](https://www.ieeg.org)
- [OpenNeuro iEEG datasets](https://openneuro.org/search/modality/ieeg) - BIDS-iEEG human ECoG/SEEG collections.
- 

## Single-Unit & Extracellular Electrophysiology

- [Allen Brain Observatory – Neuropixels](https://portal.brain-map.org/circuits-behavior/visual-coding-neuropixels) - Large-scale mouse visual system spiking data.

## Brain Stimulation (TMS / tDCS / TMS-EEG)

- [TMS-EEG datasets on OpenNeuro](https://openneuro.org/search/modality/eeg)
- [TMSEEG / TESA example data](https://nigelrogasch.github.io/TESA/)

## fNIRS

## License

[![CC0](https://licensebuttons.net/p/zero/1.0/88x31.png)](https://creativecommons.org/publicdomain/zero/1.0/)

To the extent possible under law, the contributors have waived all copyright and related or neighboring rights to this work. This list itself is released under [CC0 1.0](https://creativecommons.org/publicdomain/zero/1.0/). Individual datasets retain their own licenses and access terms — always check before use.
